...
export async function generateMetadata(
    ...
    if (!cat) {
        return {
            ...
            description: 'The requested category could not be found on Squad Busters.',
            robots: {
                index: false,
                follow: false,
            },
    ...
    const description = `Play ${cat.name.toLowerCase()} puzzles on Squad Busters. ${cat.description} ${cat.levels.length} levels from easy to expert.`;
    const canonicalUrl = `/categories/${cat.slug}`;

    return {
        title,
        description,
        keywords: [`${cat.name.toLowerCase()} puzzles`, `${cat.name.toLowerCase()} jigsaw`, `Squad Busters ${cat.name.toLowerCase()}`, 'free puzzle game'],
        alternates: {
